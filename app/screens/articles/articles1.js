...
export class Articles1 extends React.Component {
  ...
  _renderItem(info) { console.log(info.item.header); console.log(info.item.mediaDuration,'mediaDuration')
    return (
      <TouchableOpacity
        delayPressIn={70}
        activeOpacity={0.8}
        onPress={() => this.props.navigation.navigate('Article', {id: info.item.id, header: info.item.header, photo: info.item.photo, text: info.item.text, audioUrl: info.item.audioUrl,publishedDate:info.item.publishedDate, mediaDuration: info.item.mediaDuration})}>
